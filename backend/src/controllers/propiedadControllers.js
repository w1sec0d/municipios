...
const insertPropiedad = async (req, res) => {
   ...
   //llamado para insertar un propiedad.
    ...
    database.query(query, (err, rows) => {
        ...
        res.send('Propiedad inserted successfully.');
    });
};
//-----------------------------------------------------------------
...
const updatePropiedad = async (req, res) => {
    ...
    const query = `UPDATE PERSONA_has_VIVIENDA SET ${fieldsToUpdate.join(', ')} WHERE PERSONA_id_persona = ${id1} AND VIVIENDA_id_vivienda = ${id2}`;
