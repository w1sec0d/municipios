...
const getEvento = async (req, res) => {
  database.query(
    "SELECT v.*, municipio.nombre AS municipio_nombre FROM EVENTO v JOIN MUNICIPIO municipio ON v.MUNICIPIO_id_municipio = municipio.id_municipio",
    (err, rows) => {
      if (err) {
        console.error(err);
        res
...
const insertEvento = async (req, res) => {
  ...
  database.query(checkMunicipioQuery, [municipio_nombre], (err, results) => {
    ...
    database.query(
      "INSERT INTO EVENTO (id_evento,MUNICIPIO_id_municipio, nombre, presupuesto, descripcion, fecha_inicio, fecha_fin) VALUES (?,?,?,?,?,?,?)",
      [
        id_evento,
        id_municipio,
        nombre,
